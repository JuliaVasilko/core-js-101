...
function findFirstSingleChar(str) {
  const uniqSymbols = [...new Set(str.split(''))];
  for (let i = 0; i < uniqSymbols.length; i += 1) {
    const mathArr = [...str.matchAll(uniqSymbols[i])];
    if (mathArr.length === 1) {
      return uniqSymbols[i];
    }
  }
...
function getDigitalRoot(num) {
  const sumNum = num.toString().split('').reduce((acc, curr) => acc + Number(curr), 0);

  if (sumNum >= 10) {
    return getDigitalRoot(sumNum);
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPaths[0].length; i += 1) {
    ...
    if (splitPaths.every((path) => path[i] === currentDir)) {
      commonPath.push(currentDir);
    } else {
      break;
    }
  }
